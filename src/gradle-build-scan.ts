...
export async function setup(): Promise<void> {
    ...
    if (!fs.existsSync(initScriptFile)) {
        await writeInitScript(initScriptFile, core.getInput(DEVELOCITY_PLUGIN_VERSION_INPUT), core.getInput(COMMON_USER_DATA_PLUGIN_VERSION_INPUT))
...
async function writeInitScript(initScriptFile: string, gradleEnterprisePluginVersion: string, commonUserDataPluginVersion: string): Promise<void> {
    const buildScanTemplate = readResourceFileAsString('build-scan.gradle')
    const templateVars = {gradleEnterprisePluginVersion: gradleEnterprisePluginVersion, commonUserDataPluginVersion: commonUserDataPluginVersion}
